    ...
    >
      <View className="flex-1 justify-end bg-black/70">
        ...
        <View
            ...
            // @ts-ignore
            style={{ boxShadow: '0px -4px 16px rgba(0, 255, 255, 0.25)', elevation: 24 }}
        >
          <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
            ...
            <View className="px-6 -mt-10">
                ...
                <View className="flex-row flex-wrap gap-2 mb-6">
                    <View className={`px-3 py-1.5 rounded-full border bg-white/5 ${
                        trick.status === 'IN_PROGRESS' ? 'border-primary' :
                        trick.status === 'COMPLETED' ? 'border-success' : 'border-white/10'
                    }`}>
                         <Text
                            className={`text-xs font-bold uppercase tracking-wider ${
                                trick.status === 'IN_PROGRESS' ? 'text-primary' :
                                trick.status === 'COMPLETED' ? 'text-success' : 'text-textDim'
                            }`}
                         >{trick.status.replace('_', ' ')}</Text>
                    </View>
                    <View className="px-3 py-1.5 rounded-full border border-white/10 bg-white/5">
                        <Text className="text-xs font-bold uppercase tracking-wider text-textDim">{trick.difficulty}</Text>
                    </View>
                    <View className="px-3 py-1.5 rounded-full border border-white/10 bg-white/5">
                        <Text className="text-xs font-bold uppercase tracking-wider text-textDim">{trick.points} PTS</Text>
                    </View>
                </View>
                ...
                {trick.prerequisites.length > 0 && (
                    <View className="mb-4">
                        <Text className="text-textDim text-sm font-bold uppercase mb-2">Prerequisites:</Text>
